...
IS_compound_to_remove <- "glucose-13C6"
...
M_t_full <- M_t_full %>% 
  ...
  filter(Compound.Name != IS_compound_to_remove)
...
for(i in 1:nrow(Cat.ms.all.q)){
  ...
  if(lri<1900){
    Cat.ms.all.q$RTI.transform[i] = (0.0342*exp(0.0059*lri))
  }else{
    Cat.ms.all.q$RTI.transform[i] = (-1.3*lri+4987)
  }
}
...
set.seed(144)
train.rf = train(slope.i.q ~., data = Quant.mod.train.t, method = "rf", tuneGrid = data.frame(mtry=seq(1, 30, 1)), trControl = trainControl(method = "cv", number = 5), metric = "Rsquared", na.action = na.exclude)
