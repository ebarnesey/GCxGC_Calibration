...
IS_goodmatch <- M_t_LRI %>% 
  ...
  filter(Library.Match.Factor > Match_factor_floor-50)# down adjusting LRI match factor due to some misses of good matches 
...
write.csv(IS_positions, "Files/IS_positions_ESmod.csv")
...
write.csv(IS_positions_avg, "Files/IS_positions_avg_ESmod.csv")
...
#write.csv(real_cc_avg.c, "Files/CC_slopes_categories_for_curve.csv")
...
for(i in 1:nrow(Cat.ms.all.q)){
  ...
  if(lri<1862){
    Cat.ms.all.q$RTI.transform[i] = (0.00000115*exp(0.0115*lri))
  }else{
    Cat.ms.all.q$RTI.transform[i] = (33600*exp(lri*-.00144))
  }
}
...
# png("normcurve.png", width=7.5, height=4,
...
for(i in 2:nrow(arranged_mz.s)){
  arranged_mz.s$loss_fast[i]= arranged_mz.s$mz[i-1]-arranged_mz$mz[i]
}
...
for(i in 1:nrow(SS_table_formod
  ...
  if(lri<1862){
    SS_table_formod$RTI.transform[i] = (0.00000115*exp(0.0115*lri))
  }else{
    SS_table_formod$RTI.transform[i] = (33600*exp(lri*-.00144))
  }
}
...
write.csv(SS_sample_quant, "Files/Write/Compiled_Compound_info_SeaScape.quant_EBF.csv")
